...
const TourFloatingBookingBar = ({ tour }: TourFloatingBookingBarProps) => {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-40 border-t border-[rgba(28,26,20,0.1)] bg-white/95 px-4 py-3 shadow-[0_-8px_32px_rgba(0,0,0,0.08)] backdrop-blur-md lg:hidden"
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
        ...
        <Button
          type="button"
          className="min-h-11 min-w-[8rem] shrink-0 cursor-pointer bg-[#c8102e] text-[15px] font-medium text-white shadow-[0_8px_24px_rgba(200,16,46,0.3)] transition hover:bg-[#a50d25]"
          onClick={() => setOpen(true)}
        >
          {t('tour.booking.book_now', 'Book now')}
        </Button>
      </div>
        ...
        <DrawerContent className="max-h-[90vh]">
          <DrawerHeader>
            <DrawerTitle className="text-left text-[#1c1a14]">
              {t('tour.booking.drawer_title', 'Book this tour')}
            </DrawerTitle>
          </DrawerHeader>
          <div
