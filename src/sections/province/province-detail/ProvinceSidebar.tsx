  ...
  const facts = [
    {
      ...
      value: province.population ? province.population.toLocaleString() : '--',
    },
    {
      label: t('province.area', 'Area'),
      value: province.area ? `${province.area.toLocaleString()} km2` : '--',
    },
    {
  ...
  const stats = [
    {
      ...
      value: province.totalHotels ?? 0,
    },
    {
      icon: Compass,
      label: t('province.total_tours', 'Tours'),
      value: province.totalTours ?? 0,
    },
    {
      icon: Users,
      label: t('province.total_tour_guides', 'Tour guides'),
      value: province.totalTourGuides ?? 0,
    },
  ];
          ...
          {stats.map((stat) => (
            <div key={stat.label} className="flex items-center gap-3 rounded-xl bg-[#faf7f2] px-3 py-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-[#1c1a14]">{stat.value.toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>
